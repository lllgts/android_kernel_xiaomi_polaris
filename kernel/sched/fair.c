...
static void throttle_cfs_rq(struct cfs_rq *cfs_rq)
{
	...
	for_each_sched_entity(se) {
		...
		walt_propagate_cumulative_runnable_avg(
				   &qcfs_rq->cumulative_runnable_avg_scaled,
				   cfs_rq->cumulative_runnable_avg_scaled, false);
	...
	if (!se) {
		...
		walt_propagate_cumulative_runnable_avg(
				   &rq->cumulative_runnable_avg_scaled,
				   cfs_rq->cumulative_runnable_avg_scaled, false);
	}
...
void unthrottle_cfs_rq(struct cfs_rq *cfs_rq)
{
	...
	for_each_sched_entity(se) {
		...
		walt_propagate_cumulative_runnable_avg(
				   &cfs_rq->cumulative_runnable_avg_scaled,
				   tcfs_rq->cumulative_runnable_avg_scaled, true);
						    ...
						    u64 new_task_load)
{
	...
	for_each_sched_entity(se) {
		...
		cfs_rq->cumulative_runnable_avg_scaled += task_load_delta;
		if (cfs_rq_throttled(cfs_rq))
			break;
	}
...
static inline bool task_fits_max(struct task_struct *p, int cpu)
{
	...
	if (task_boost_policy(p) == SCHED_BOOST_ON_BIG || walt_should_kick_upmigrate(p, cpu))
		return false;
...
static int get_start_cpu(struct task_struct *p)
{
	...
	if (task_skip_min || boosted) {
		start_cpu = rd->min_cap_orig_cpu == -1 ?
...
bias_to_waker_cpu(struct task_struct *p, int cpu, int start_cpu)
{
	...
	return cpumask_test_cpu(cpu, tsk_cpus_allowed(p)) &&
	       ...
	       capacity_orig_of(cpu) >= capacity_orig_of(start_cpu);
}
...
static inline bool
bias_to_prev_cpu(struct task_struct *p, int start_cpu)
{
	int prev_cpu = task_cpu(p);
#ifdef CONFIG_SCHED_WALT
...
static int select_energy_cpu_brute(struct task_struct *p, int cpu, int prev_cpu, int sync)
{
	...
	if (bias_to_prev_cpu(p, start_cpu)) {
		target_cpu = prev_cpu;
...
out:
	trace_sched_task_util(p, next_cpu, backup_cpu, target_cpu, sync,
			      ...
			      fbt_env.placement_boost,start_t, boosted, is_rtg, get_rtg_status(p),
			      start_cpu);
	return target_cpu;
}
